...
try {
  logopdf = fs.readFileSync(logopdfPath).toString('base64');
} catch (error) {
...
export const generatePDF = async (invoice: IInvoice): Promise<Buffer> => {
  ...
  await populatedInvoice.populate('products.productId');
    ...
    // Create product rows for the table
    ...
    const content = `
      ...
      <body style="width: 210mm; min-height: 297mm; background-color: white; padding: 40px;">
        ...
        <div style="
            background: 
              radial-gradient(55.35% 63.52% at 53.55% 87.4%, #0A0A0A 0%, #1D1F3A 100%),
              radial-gradient(116.08% 116.08% at 50.43% 51.58%, #0C0C0C 0%, rgba(0, 0, 0, 0) 100%),
              linear-gradient(0deg, #0B3A99, #0B3A99);
            border-radius: 12px; 
            padding: 24px; 
            color: white; 
            margin-bottom: 12px;
        ...
        <div style="margin-bottom: 32px;">
          <div style="background: linear-gradient(90deg, #303661 0%, #263406 100%); border-radius: 78px; padding: 12px 16px; display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="color: white; font-size: 14px; flex: 2; text-align: left;">Product</span>
            <span style="color: white; font-size: 14px; flex: 1; text-align: center;">Qty</span>
            <span style="color: white; font-size: 14px; flex: 1; text-align: center;">Rate</span>
            <span style="color: white; font-size: 14px; flex: 1; text-align: right;">Total Amount</span>
          </div>
